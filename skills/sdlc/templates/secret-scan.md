...
**Policy — warn-only, never blocks the hand-off**:
- Any finding (HIGH, MEDIUM, LOW, or regex-fallback match) → record file
  and line, surface it in the Stage 7 report, and **proceed** to the hand-off.
  The scan never blocks; the user decides before committing.
...
Record the scan tool used and finding count in the Stage 7 report so the reader knows a scan ran.
