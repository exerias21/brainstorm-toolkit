...
| `next_action` | object | optional | Additive (L8 — the pending handoff, durable). `{"cmd": "...", "confirm": bool}`, mirroring a `.next-action` sentinel line, recording the next step this run proposed when it finished/paused. Lets `/next` and `/status` recover the proposed handoff from the envelope even after the fire-once sentinel was consumed — the loop's "program counter" survives in a file, not just chat context. Absent when the run proposed no next action. |
