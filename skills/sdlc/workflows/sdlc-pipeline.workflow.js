...
never fail). Otherwise return demoted_lenses = every lens name whose entry has demoted===true.`,
  ...
  // agents.code_review_lenses selects WHICH lenses; agents.code_review_max_lenses caps HOW MANY.
  // Truncation runs AFTER the demotion filter so a demoted lens can never consume one of the
  // allowed slots, and in list order so a max_lenses:1 run keeps the highest-yield lens
  // (`correctness` by default order). Guard the value like every other malformed cfg read: a
  // non-integer or non-positive value falls through to the default rather than yielding an empty
  // fan-out, which would silently DISABLE the stage instead of failing it.
  const CONFIGURED_LENSES = cfg.agents?.code_review_lenses ?? DEFAULT_LENSES
  const rawMaxLenses = cfg.agents?.code_review_max_lenses
  const MAX_LENSES = Number.isInteger(rawMaxLenses) && rawMaxLenses > 0 ? rawMaxLenses : 4
  const REVIEW_LENSES = CONFIGURED_LENSES
  ...
  log(
    `review lenses: ${REVIEW_LENSES.join(', ') || '(none)'} (${REVIEW_LENSES.length} of ${DEFAULT_LENSES.length} defaults` +
      ...
      (REVIEW_LENSES.length < CONFIGURED_LENSES.filter((l) => !demotedLenses.includes(l)).length
  ...
  // highest-priority hazard on this axis, see templates/models.md).
  if (MODEL_CAP && TIER_NAMES.includes(REVIEW_MODEL) &&
      TIER_NAMES.indexOf(REVIEW_MODEL) > TIER_NAMES.indexOf(MODEL_CAP)) {
    log(`review: reviewer runs ${REVIEW_MODEL} on ${REVIEW_LENSES.length} lens(es) + verify + fix-planner. models.cap (${MODEL_CAP}) does NOT govern this axis -- lower it with models.code_review, or cut the fan-out with agents.code_review_max_lenses. See templates/models.md.`)
  }
