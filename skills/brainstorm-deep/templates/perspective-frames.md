...
In a **single message**, fire one Agent tool call per selected frame. Use `subagent_type: "general-purpose"`. Each agent's prompt is the frame's "Intent" + the bolded prompt above + the agreed framing + clarification answers.

Wait for all to return, then compile their outputs into the plan's `## Perspective passes` section, one subsection per frame, in the order they appear in the `--frames` list (or default order).
