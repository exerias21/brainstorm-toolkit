...
name: sdlc-lite
description: >
  Sequential full-pipeline-minus-PR skill for Copilot. Takes a plan file, a
  ...
  current branch. No branch creation, no push, no PR. Copilot-optimized
  overlay of canonical /sdlc-lite — every stage runs inline (no parallel
  sub-agents, no Plan mode). Same stages as /sdlc; only the ending differs.
...
difference is Stage 6 — commit on the current branch instead of opening a PR.
