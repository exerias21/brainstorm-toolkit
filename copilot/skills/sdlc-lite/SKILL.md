...
runs every stage inline, one at a time. Same stages as `/sdlc-lite`; the only
difference is Stage 6 — `/sdlc-lite` does **no git writes** (it hands you a
validated tree to commit), while `/sdlc-lite` commits + opens a PR.
...
Same procedure as `/sdlc-lite` Stage 3 (see `.github/skills/sdlc/SKILL.md`):
