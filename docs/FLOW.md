...
flowchart TD
    ...
    PICK -->|"one bounded fix"| TASK["/task — TDD red→green on current branch; commits only if you ask"]
...
|---|---|---|---|
| `/task <desc>` | ad-hoc ask | TDD red→green, working tree left for you; **commits only when you ask** | a one-line fix, a small util, a rename — no evals/flowsim |
...
- **Reviewer axis (shipped, opt-in).** The Review→Fix stage adds a *separate* reviewer model
  ...
  design history: [`REVIEW-FIX-STAGE.md`](REVIEW-FIX-STAGE.md).
...
- [`REVIEW-FIX-STAGE.md`](REVIEW-FIX-STAGE.md) — historical design record for the adversarial Review→Fix stage and the reviewer-model axis. The stage shipped; its live contract is `skills/sdlc/templates/stage-5.7-review-fix.md`.
