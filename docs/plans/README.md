...
| 6 | `board-json-export.md` | A read-only `close-tasks.sh board` subcommand exporting `TASKS.md` + pipeline-envelope state as one JSON object, for a cross-machine/dashboard consumer with no new store. | Low — no model spend |
| 7 | `dogfood-followups.md` | Lint/contract-check fixes plus skill-repo fixes found dogfooding the toolkit on itself: `_manual_`/`_followup_` row tags, a phase-aware `reconcile`, and the `/brainstorm`/`/flowsim` skill-repo plan location (`docs/plans/`) and its knock-ons. | Low — no model spend |
...
| 9 | `subagent-git-guard.md` | Guards against a dispatched sub-agent making a git write the orchestrator didn't ask for. | Low — no model spend |
