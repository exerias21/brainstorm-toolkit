...
- `.agents/skills/<name>/SKILL.md` for every cross-tool skill, with Codex-tuned overlays for `/sdlc` and `/tasker` (sequential, no Plan mode).
...
```
   ...
   └──────────────────────────────────────┘
        or
   ┌─ Mid-sized task ─────────────────────┐
   │  /tasker "wire orders into the queue"│
...
Codex CLI exposes installed skills as slash commands the same way Copilot does — `/sdlc plans/foo.md`, `/task add hello util`, `/tasker task-007`. The runtime contract (frontmatter parsing, `argument-hint`, tool gating) follows the Codex CLI Agent Skills spec; see the upstream Codex CLI docs for the canonical reference. Our skills declare `metadata.brainstorm-toolkit-applies-to: codex` (or include `codex` in a multi-tool list) so `setup.sh` routes them into `.agents/skills/`.
