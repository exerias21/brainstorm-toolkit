...
name: tasker
description: >
  Sequential task-row pipeline for Codex CLI. Resolves or creates a TASKS.md
  ...
  No branch creation, no push, no PR. Codex-optimized overlay of the
...
# Tasker (Codex Edition — Sequential)
...
Codex CLI's 2026 Agent Skills spec, like Copilot's, doesn't yet support parallel sub-agent dispatch. This overlay bootstraps from the Copilot overlay and tracks it closely; if Codex-specific behavior diverges, this file can be tuned independently.
